...
const validMMDS = `{
  ...
  "repo": {"slug":"acme/foo","clone_url":"https://github.com/acme/foo.git","base_branch":"main"},
  "task": {"mode":"initial","prompt":"add a healthcheck"},
  "llm": {"base_url":"http://172.31.0.1:8080","auth_token":"sess-abc"},
  "github": {"token":"ghs_xxx","bot_user":"acme-agent[bot]","bot_email":"bot@users.noreply.github.com"},
...
func TestParseBootConfigRejectsInvalid(t *testing.T) {
	cases := map[string]string{
		...
		"missing repo": strings.Replace(validMMDS, "https://github.com/acme/foo.git", "", 1),
