...
thing branch protection discriminates from a human. Rules 4 and 5 below are
the real safety guarantee: enforced by GitHub, not by our code being correct.
...
**Migrate the target repos to your GitHub organization before this step**.

## 1. Create the App (your GitHub organization → Settings → Developer settings → GitHub Apps → New)
...
| Checks | Read-only | consume CI results (e2e runs on GitHub CI → `ci_status`) |

**Do NOT grant** `actions`, `administration`, `workflows`, or any org-level permission.
Checks:read is read-only and is the only addition to the minimal permission set.

**Subscribe to webhook events:** `pull_request`, `pull_request_review`,
`pull_request_review_comment`, `issue_comment`, `check_suite`.

**Webhook URL:** point at `webhook-rx` (the webhook receiver). Set a webhook secret and store it on the
control plane (Tier-0). You can leave the webhook inactive for now.

## 2. Generate and store the private key

Generate a private key from the App page. It is **Tier-0** — it never enters a guest and
lives only on the control plane, where the credential minter exchanges it for short-lived
...
- **Exclude the App from being able to approve** — the App must never satisfy its own review
  gate. (Reviews from the App/bot don't count toward required approvals.)
...
cannot get agent output onto `main` — GitHub rejects the push.
...
The minter issues a 1-hour installation token (`contents:write` + `pull_requests:write`,
...
restart. The bot commit identity is `<app-slug>[bot]` / `<id>+<app-slug>[bot]@users.noreply.github.com`,
