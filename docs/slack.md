...
Then in your channel: `/fleet acme/hello-gents add a CHANGELOG.md`. Within a few seconds a
thread appears.
...
- `/fleet acme/hello-gents add a CONTRIBUTING guide` — dispatch on the default model (Sonnet).
- `/fleet --cheap acme/hello-gents fix the lint warnings` — dispatch on the cheap model (Haiku),
  for boring/mechanical work.

You get a private (ephemeral) acknowledgement, then the public thread opens in the same channel:

```
(only you see this)  Dispatched s_RX94… on acme/hello-gents — I'll open a thread here.
```
...
```
you:  /fleet acme/hello-gents add a LICENSE file (MIT, 2026, acme)

#fleet
┌─ 🤖 Task dispatched  s_RX94WFYM9GSV6J7N07SAP3B40T
│     repo acme/hello-gents   branch agent/s_RX94…
│     > add a LICENSE file (MIT, 2026, acme)
...
```
you:  /fleet --cheap acme/hello-gents normalize all line endings to LF

┌─ 🤖 Task dispatched  s_8QP2…
│     repo acme/hello-gents   branch agent/s_8QP2…
...
```
you:  /fleet acme/api add pagination to the /users endpoint
