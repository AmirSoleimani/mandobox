#!/usr/bin/env bash
# gen-dev-certs.sh — generate a self-signed dev PKI for fleet-agent mTLS.
#
# FOR M2 TESTING ONLY. Not a real CA: one CA signs both the server (fleet-agent) and the
# client (fleet-reconciler / control plane). Replace with your real PKI before production.
#
...
#   server.crt server.key client-ca.crt   (fleet-agent)
#   reconciler.crt reconciler.key server-ca.crt  (fleet-reconciler)
set -euo pipefail
...
echo "gen-dev-certs: server (fleet-agent), SAN=${SAN}"
openssl req -newkey rsa:4096 -nodes -keyout server.key -out server.csr \
  -subj "/CN=fleet-agent" 2>/dev/null
openssl x509 -req -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial \
  -out server.crt -days "$DAYS" \
  -extfile <(printf "subjectAltName=%s\n" "$SAN") 2>/dev/null

echo "gen-dev-certs: client (fleet-reconciler)"
openssl req -newkey rsa:4096 -nodes -keyout reconciler.key -out reconciler.csr \
  -subj "/CN=fleet-reconciler" 2>/dev/null
